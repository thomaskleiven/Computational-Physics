...
//Declare variables
static double endtime = 0.08;

static int nx = 201;                       //Number of length steps

static double dx = 1.0/(nx+1);                //Size of length steps
static double dt = 1/1E5;           //Size of time steps
...
void implicitEuler(bool reflective){
  ...
  void crankNicolson(bool reflective){
    ...
    generateA(Scheme_t::CRANK_NICOLSON, D);
    ...
    for(int i = 0; i < nt; ++i){
      ...
      u(0) = u(0)*diagonalB(0) + subDiagonalB(0)*u(1);
      u(u.n_elem-1) = u(u.n_elem-1)*diagonalB(diagonalB.n_elem-1) + u(u.n_elem-2)*subDiagonalB(subDiagonalB.n_elem-1);

      //Solve the system Ax = Bb
      dpttrs_(&nx, &nrhs, diagonal.memptr(), subDiagonal.memptr(), u.memptr(), &nx,  &info);

      if(abs(0.002-i*dt) <1E-20){
        results.col(counter++) = u;
      }
    }
    //stringstream fname;
